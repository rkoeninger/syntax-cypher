var del = require("del"),
    ...
    lsFiles = "./src/**/*.ls",
    buildRoot = "./build",
    jsFiles = buildRoot + "/**/*.js",
    jsMainFiles = buildRoot + "/main/**/*.js",
    jsTestFiles = buildRoot + "/test/**/*.js",
    distRoot = "./dist";

task("build", () => src(lsFiles).pipe(lsc).pipe(dest(buildRoot)));

task("bundle", ["build"], () => src(jsMainFiles).pipe(webpack).pipe(dest(distRoot)));

task("clean", () => del([buildRoot, distRoot]));
